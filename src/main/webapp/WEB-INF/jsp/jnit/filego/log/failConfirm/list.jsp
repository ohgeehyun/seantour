...
<form:form commandName="searchVO" name="filegoListForm" id="filegoListForm" method="post">
	<div id="filegoFailConfirmDiv">
		<table class="table table-bordered table-striped table-hover" summary="배포실패확인 로그">
			<tr>
				<th style="width: 5%;">No</th>
				<!-- <th>UUID</th> -->
				<th style="width: 8%;">보낸 서버</th>
				<th style="width: 12%;">받는 서버</th>
				<th style="width: *;">파일명</th>
				<th style="width: 15%;">메시지</th>
				<th style="width: 13%">전송 실패일자</th>
				<th style="width: 13%;">확인 일자</th>
				<th style="width: 8%;">확인자</th>
			</tr>
			<c:if test="${fn:length(resultList) < 1}">
				<tr>
					<td colspan="8" style="text-align: center;">내역이 존재하지 않습니다.</td>
				</tr>
			</c:if>
			<c:forEach var="result" items="${resultList}" varStatus="status">
				<tr>
					...
					<td style="text-align: center; vertical-align: middle;"><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${result.confirmed}"/></td>
					<td style="text-align: center; vertical-align: middle;"><c:out value="${result.confirmUserId}" /></td>
				</tr>
			</c:forEach>
		</table>
	</div>
