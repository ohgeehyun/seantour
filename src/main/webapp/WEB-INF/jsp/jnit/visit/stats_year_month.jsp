		...
		<div class="row-fluid">
           	<div class="span4">
           		<div class="widget-box">
               		<div class="widget-title" style="height: 0%;">
						<form id="search_form" name="search_form">
							...
							<c:if test="${day == 1}">
								<input name="search_day1" id="search_day" type="text" class="datepicker" value="<c:out value="${searchDateStr}" />" style="width: 100px;"/>
							</c:if>
							<c:if test="${day == 2}">
                    ...
                    <div class="widget-content nopadding">
						<table class="table table-bordered table-striped table-hover" summary="연간 월별 방문자">
							<thead>
								<tr>
									<th>시간</td>
									<th>방문수</td>
							...
							<tbody>
								...
								<tr>
									<td align="center"><c:out value="${result.vdate}" escapeXml="true" />월</td>
...
<script>
	...
	var listSize = ${fn:length(resultList)};
...
g.set_x_labels( [<c:forEach var="stats" items="${resultList}" varStatus="status">
					'${fn:substring(stats.vdate,5,7)}월'<c:if test="${!status.last}">,</c:if>
				</c:forEach>] );
...
g.pie_values( data, [<c:forEach var="stats" items="${resultList}" varStatus="status">
					...
					'${fn:substring(stats.vdate,5,7)}월',
					</c:if>
					<c:if test="${status.count == fn:length(resultList)}">
					'${fn:substring(stats.vdate,5,7)}월'
					</c:if>
