...
<%
	...
	if(request.getParameter("allowedTypes")!=null){
		allowedTypes = (String) request.getParameter("allowedTypes");
	}
...
<script src="<c:url value="/resources/js/fileupload/jquery-1.9.1.js" />"></script>
...
function fileDown(chkFileObj){
	...
	for(i=0; i<chkFileObj.length; i++){
		...
		$("#fileFormArea"+i).attr("src","<c:url value='/cms/fileupload/fileDownload.do' />"+"?fileId="+chkFileObj[i].value);
	}
}
	...
	if(chkFileObj==null || chkFileObj.length==0){
		...
		if(confirm("선택하신 파일을 삭제하시겠습니까?")){
			for(i=0; i<chkFileObj.length; i++){
				...
				$.ajax({
					url:  '<c:url value="/cms/fileupload/deleteFile.do" />',
					type: 'POST',
					data: param,
					dataType:"json",
					async: false,
...
function getFileInfo(fileId){
	...
	$.ajax({
	    url:  '<c:url value="/cms/fileupload/getAttachedFileInfo.do" />',
	    type: 'POST',
	    data: param,
		dataType:"json",
		async: false,
	    success: function(data){
			returnObj = data.attachedFileVO;
	    },
	    error: function(e){
	        alert(e.reponseText);
		},
...
function getFileInfoList(fileId){
	var returnObj = new Object();
	...
	$.ajax({
	    url:  '<c:url value="/cms/fileupload/getAttachedFileInfoList.do" />',
	    type: 'POST',
	    data: param,
		dataType:"json",
		async: false,
	    success: function(data){
			returnObj = data.attachedFileVOList;
	    },
	    error: function(e){
	        alert(e.reponseText);
		},
...
$(document).ready(function() {
	settings = {
		url: '<c:url value="/cms/attachedFile/attachFileUpload.do" />',
		method: "POST",
		returnType:"json",
		...
		onSelect: function(files){
			...
			var isValid = true;
			if (!(/<%=allowedTypes1%>/i).test(uploadFile.name)) {
				alert('<%=allowedTypes%> 파일 만 가능합니다');
				isValid = false;
			} else if (uploadFile.size > 500000000) { // 500mb
				alert('파일 용량은 <%=maxFileSize%> 를 초과할 수 없습니다.');
				isValid = false;
			}
			if(!isValid) {
		...
		onSuccess:function(files,data,xhr){
			...
			if(code == '1') {
				...
				var fileObj = getFileInfo(fileId);

				var vImgIcon = "";
				var arrFileKind = fileObj.viewFileName.split(".");
				...
				var vHtml	= ' <tr class="upload_file" id="tr_upload_file_'+fileObj.fileId+'"> '
							...
							+ ' <td class="fileup_name"><label for="upload_file_'+fileObj.fileId+'"><img src="'+vImgIcon+'" alt="" /> '+fileObj.viewFileName+'</label></td> '
							+ ' <td class="fileup_size">'+fileObj.fileSize+' Byte</td> '
							+ ' </tr> ';
	...
	$("#uploadfile").uploadFile(settings,1);
	$("#mulitplefileuploader").uploadFile(settings,2);
});

$(document).ready(function() {
	var fileObjList = getFileInfoList("<%=fileId%>");
	if(fileObjList!=null){
...
<div class="fileUploader" id="no_upload_file0" style="display:none">
	<div class="fileUploader_info">
        <button class="upload_file"><div id="uploadfile">파일 추가</div></button>
        <button class="delete_file" onClick="javascript:fn_deleteFile();">삭제</button>
        <button class="upload_file" onClick="javascript:fn_downloadFile();">다운로드</button>
		<!--span class="upload_guide">파일 3/3개 | 2 MB/10MB</span-->
    </div>
...
<div id="file_upload_status" class="file_transferring" style="display:none">
	<div class="filetrans_header">
    	<p class="top_text">파일을 업로드 하는 중 입니다.</p>
...
!function (e) {
	...
	e.fn.uploadFile = function (t, vKind) {
		...
		if(vKind==1){
			...
			var C = this,
			b = e("<div class='upload_info'>마우스로 원하는 파일을 넣어주세요</div>");
		}else if(vKind==3){
			var C = this,
		...
		function F() {
			if (e.fn.ajaxForm) {
				...
				w.showQueueDiv ? C.container = e("#" + w.showQueueDiv) : C.container = e("<div></div>").insertAfter(e(C)),
