...
<%
	...
	data = (JSONObject) new JSONParser().parse(AdminUtil.CmsMenuTree(loginVO.getTypeId()));
...
<script>
	$(document).ready(function(){
		...
		$('#typeId').change(function(){
			...
			if($.trim($("#typeId").val()) != ''){
				var typeId = $(this).val();//$("#typeId").val();
				$.ajax({
					type: "POST",
					url: "<c:url value="/cms/sub2/menuTreeGet.do"/>",
					data: "typeId="+typeId
				}).done(function(data){
					...
					for(var i=0; i<split.length; i++){
						menuId = split[i];
						$("input[value='"+menuId.trim()+"']").parent().addClass("checked");
						$("input[value='"+menuId.trim()+"']").attr("checked",true);
					}
				});
				$("#slide").slideDown();
			}else{
		...
		$('#regist').click(function(){
			...
			var check = $("input[name='powerList']").is(":checked");
            	...
            	<div class="span12">
                	...
                	<form id="configForm" name="configForm" method="post" action="<c:url value='/cms/sub2/020101save.do'/>">
                        ...
                        <div class="widget-content nopadding">
                               <table class="table table-bordered table-striped th_left" summary="회원유형">
                                   <tbody>
                                       <tr>
											<th colspan="3" style="text-align: center;">
												<strong>*</strong> <label>회원유형</label><a href="javascript:Jnit_menu_question('1');"><i class="icon-question-sign"></i></a>
												
	                                       		<div id="JnitMenuQuestion1" style="display: none;">
													<strong>&nbsp;- 각 유형별로 해당 관리자 모드에 접근시 보여지는 메뉴를 제어하는 (노출/비 노출) 페이지입니다.</strong><br />
													<strong>&nbsp;- 통합관리자 &gt; 회원관리 &gt; 유형관리 페이지 링크</strong>
												</div>
											</th>
                                       </tr>
                                       <tr>
                                           <td colspan="3" style="text-align: center;">
	                                           <select id="typeId" name="typeId">
	                                           	...
	                                           	<c:forEach var="typeList" items="${typeNm}">
	                                           		<c:if test="${typeList.typeId != '30020' }">
	                                     	      		<option value="${typeList.typeId}">${typeList.typeNm} / ${typeList.typeLv}</option>
	                                           		</c:if>
	                                           	</c:forEach>
	                                           </select>
											</td>
