...
function addFolNode(node) {
	...
	if(node.data.isFolder == true) {
		...
		$.ajax({
			  type: "POST",
			  url: "<c:url value="/cms/sub6/0101add.do" />",
			  data: {
				  menuNm: "새 메뉴",
				  iscnt: "0",
   ...
   .keydown(function(event){
     ...
     if( title != prevTitle ) {
     	// DB 처리
  		$.ajax({
  			  type: "POST",
  			  url: "<c:url value="/cms/sub6/0101edit.do" />",
  			  data: {
...
function delNode(node) {	
	...
	if(childNodeCount > 0) {
   	...
   	if(confirm(node.data.title + ' ' + (node.data.isFolder ? '메뉴를' : '콘텐츠를') + ' 정말 삭제하시겠습니까?')==true) {
   		...
   		$.ajax({
 			  type: "POST",
 			  url: "<c:url value="/cms/sub6/0101del.do" />",
...
function rebuildTreeJson() {
	...
	$.ajax({
		url:"<c:url value="/cms/sub6/0101getJson.do"/>",
		data:{
			rebuild:"true"
		},
   ...
   dnd: {
     ...
     onDrop: function(node, sourceNode, hitMode, ui, draggable) {
       ...
       if(node.data.menuid != sourceNode.data.menuid) {
   		  $.ajax({
  			  type: "POST",
  			  url: "<c:url value="/cms/sub6/0101editMethod.do" />",
  			  data: {
...
var loadEditorForm = function(node) {
	...
	formUrl = "<c:url value="/cms/sub6/0101editForm.do" />";
		...
		<div class="list_content2">
			...
			<div class="content_in">
					<%--
				     ...
				     --%> 
				<div id="tree" style="width: 350px; height: 746px;"> </div>
			</div>
		...
		<div class="view_content2">
			...
			<div id="myModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
				  ...
				  <div class="modal-body">
					<div id="mbrSearch" style="display:none;"></div>
				  </div>
				</div>
