...
<div class="${grpValue.groupClassName}">
	<ul class="allimiControl">
	  <li><a href="#none"   onclick="popupZoneStop2('p');" onkeypress="popupZoneStop2('p');"  ><img src="${pageContext.request.contextPath }/{Jnit_sitePath }/common/img/Jnit_btn_stop.gif" alt="일시정지" /></a></li>
	  <li><a href="#none"   onclick="popupZoneStop2('s');" onkeypress="popupZoneStop2('s');" ><img src="${pageContext.request.contextPath }/{Jnit_sitePath }/common/img/Jnit_btn_play.gif" alt="재생" /></a></li>
	</ul>
	<ul class="allimiNum">
		<c:forEach var="allim" items="${grpResult}" varStatus="status">
		<li style="float: left; display: inline;">
			...
			<img src="${empty allim.itemUrlPath ? pageContext.request.contextPath : ''}${empty allim.itemUrlPath ? fn:escapeXml(allim.itemUrl) : allim.itemUrlPath}" alt="${DBType == 'ORACLE' || DBType == 'TIBERO' ? itemAltMap[allim.itemId]  : allim.itemAlt}" width="${allim.groupWidth}" height="${allim.groupHeight}" id="popup_zone${status.count }" /></a></p>
		</li>	 	
		</c:forEach>
	</ul>
</div>