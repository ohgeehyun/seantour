	...
	<ul class="allimiNum">
		<c:forEach var="allim" items="${grpResult}" varStatus="status">
			<li style="float: left; display: inline;">
				...
				<img src="${pageContext.request.contextPath}${empty allim.itemUrlPath ? fn:escapeXml(allim.itemUrl) : allim.itemUrlPath}" alt="${DBType == 'ORACLE' || DBType == 'TIBERO' ? itemAltMap[allim.itemId]  : allim.itemAlt}" width="${allim.groupWidth}" height="${allim.groupHeight}" id="popup_zone${status.count}" /></a></p>
			</li> 
		</c:forEach>
	</ul>
</div>