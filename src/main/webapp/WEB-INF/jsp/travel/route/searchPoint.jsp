...
<div class="mainDiv">
    ...
    <div id="divtop">
        <h3>
            ...
            <span class="top_tit">여행지 통합 검색</span>
        </h3>
    </div>
    <div id="divcont">
        <div id="divSearch">
...
function setAddrResultEl(arrResultArr) {
	...
	strHtml+='				<div class="th"><span class="label sel" onclick="setAddress(\''+ newRoadAddr +'\', \''+ latitude +'\', \''+ longitude +'\');">선택</span></div>';
	strHtml+='			</li>';
	strHtml+='			<li class="row">';
...
function setPoiResultEl(arrResultArr) {
	...
	strHtml+='				<div class="th"><span class="label sel" onclick="setAddress(\''+ arrResultArr.name +'\', \''+ arrResultArr.frontLat +'\', \''+ arrResultArr.frontLon +'\');">선택</span></div>';
	strHtml+='			</li>';
	strHtml+='		</ul>';
	strHtml+='	</td>';
	strHtml+='</tr>';
...
function setAddress(roadFullAddr, latitude, longitude) {
	if(typeof(window.opener) != "undefined") {
		window.opener.addrCallBack(roadFullAddr, latitude, longitude, '${status}', '${idx}');
		window.self.close();
	}			
}
...
$("#searchCondition").on("change", function() {
	...
	if($(this).val() > 0) {
		$("#inpKeyword").attr('placeholder', '예) 을지로 65, 을지로2가 11');
		targetUrl += 'geo/postcode';
	} else {
		$("#inpKeyword").attr('placeholder', '예) 을지로, 바다식당');
		targetUrl += 'pois';
	}
