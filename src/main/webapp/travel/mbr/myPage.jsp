...
<%@ include file='/_common/header.jsp' %><c:set var="Jnit_sitePath" value="travel" />
...
<%	
	...
	try{
		prop = ConfigUtil.getProp("SITE_00007");/*필수 입력 siteId*/ 
	}catch(Exception e){
		log.error(e.getMessage());
	} 
	String error = (String)prop.get("error");
%>
<%@ page import="jnit.scrap.ScrapUtil" %>
<%@ page import="jnit.login.log.loginlogUtil" %>
<%@ page import="java.util.ArrayList" %>
<%@ page import="egovframework.rte.ptl.mvc.tags.ui.pagination.PaginationInfo" %>
<% 
	String scrapPageNo = request.getParameter("scrapPageNo");
	String logPageNo = request.getParameter("logPageNo");
	String tabs = request.getParameter("tabs");
	String tab = request.getParameter("tab");
	int scrapStartNum = ScrapUtil.scrapStartNum(scrapPageNo);
	int logStartNum = loginlogUtil.listIPStartNum(logPageNo);
	ArrayList scrapList = new ArrayList<String>();
	scrapList = ScrapUtil.scrapList(scrapPageNo);
	PaginationInfo scrapPageNavi = new PaginationInfo();
	scrapPageNavi = ScrapUtil.scrapPageNavi(scrapPageNo);
	ArrayList loginLogList = new ArrayList<String>();
	loginLogList = loginlogUtil.listIP(logPageNo);
	PaginationInfo loginlogPageNavi = new PaginationInfo();
	loginlogPageNavi = loginlogUtil.pageNavi(logPageNo);
	String errorCode = ConfigUtil.getErrorCode("travel","myPage","G","SITE_00007");
%>
								...
								<c:if test="${!empty scrapList }">
									<c:forEach var="result" items="${scrapList}" varStatus="status">
										<tr rel="${result.scrapId}">
											...
											<td><a href="javascript:delScrap('${result.scrapId}');"><img src="${pageContext.request.contextPath }/travel/common/img/scrap_del.gif" alt="스크랩 삭제" /></a></td>
										</tr>
									</c:forEach>
								</c:if>
								<c:if test="${empty scrapList }">
