<% response.setHeader("Cache-Control","no-store"); response.setHeader("Pragma","no-cache"); response.setDateHeader("Expires",0); if (request.getProtocol().equals("HTTP/1.1")) { response.setHeader("Cache-Control", "no-cache"); } %><%@ page contentType="text/html; charset=utf-8" %><%@ include file='/_common/header.jsp' %><c:set var="Jnit_sitePath" value="travel" /><%@ page import='java.util.Properties' %><%@ page import='net.sf.json.JSONObject' %><%@ page import='jnit.cms.config.ConfigUtil' %><%	String type = request.getParameter("type"); JSONObject prop = new JSONObject(); try{prop = ConfigUtil.getProp("SITE_00007"); /*필수 입력 siteId*/ }catch(Exception e){log.error(e.getMessage());} String error = (String)prop.get("error"); String errorCode = ConfigUtil.getErrorCode("travel","signupAdd","E","SITE_00007"); String authDefaultYn = request.getParameter("authDefaultYn"); %><c:set var="siteId" value="SITE_00007" /><%@ include file='/travel/_tpls/B/TPL_000111_S.jsp' %><% /*<!-- [-CONTENT-] -->*/ %><%
/**
 *@version 3.2.0.1
 **/
%>
