...
*/%>
			<div id="tx_sidebar" class="tx-sidebar">
				...
				<div class="tx-sidebar-boundary">
					...
					<ul class="tx-bar tx-bar-left tx-nav-attach">
<%--
<c:if test="${bbsinfoVO.useMovie == 1 || bbsinfoVO.useMovie == null}">
						<li class="tx-list">
							<div unselectable="on" id="tx_movie" class="tx-movie tx-btn-trans">
								<a href="javascript:;" title="동영상" class="tx-text">동영상</a>
							</div>
						</li>
</c:if> --%>
<c:if test="${boardinfoVO.useImage == 1}">
						<li class="tx-list">
							<div unselectable="on" id="tx_image" class="tx-image tx-btn-trans">
								<a href="javascript:;" title="사진" class="tx-text">사진</a>
							</div>
						...
						<div id="tx_richtextbox_menu" class="tx-richtextbox-menu tx-menu">
							...
							<div class="tx-menu-footer">
								<img class="tx-menu-confirm"
									 src="<c:url value="/board/_editor" />/images/icon/editor/btn_confirm.gif?rv=1.0.1" alt="확인"/>
								<img class="tx-menu-cancel" hspace="3"
									 src="<c:url value="/board/_editor" />/images/icon/editor/btn_cancel.gif?rv=1.0.1" alt="취소"/>
							</div>
						</div>
					</li>
					<li class="tx-list">
	...
	<div id="tx_canvas" class="tx-canvas">
		<div id="tx_loading" class="tx-loading"><div><img src="<c:url value="/board/_editor" />/images/icon/editor/loading2.png" width="113" height="21" align="absmiddle" alt="로딩중입니다."/></div></div>
	...
	<div id="tx_resizer" class="tx-resize-bar">
		...
		<img id="tx_resize_holder" src="<c:url value="/board/_editor" />/images/icon/editor/skin/01/btn_drag01.gif" width="58" height="12" unselectable="on" alt="" />
	</div>
				<!-- 편집영역 끝 -->
			<!-- 첨부박스 시작 -->
				<!-- 파일첨부박스 Start -->
	<div id="tx_attach_div" class="tx-attach-div">
		...
		<div id="tx_attach_box" class="tx-attach-box">
			<div class="tx-attach-box-inner">
				<div id="tx_attach_preview" class="tx-attach-preview"><p></p><img src="<c:url value="/board/_editor" />/images/icon/editor/pn_preview.gif" width="147" height="108" unselectable="on"/></div>
...
<script type="text/javascript">
	var formName = "bbsForm";
	if($('form[name="bbsForm"]').length==0){
		if($('form[name="culttour"]').length>0){
			formName = "culttour";
		}else if($('form[name="cultshop"]').length>0){
			formName = "cultshop";
		}else if($('form[name="cultshop"]').length>0){
			formName = "cultshop";
		}else if($('form[name="cultfood"]').length>0){
			formName = "cultfood";
		}else if($('form[name="cultfstv"]').length>0){
			formName = "cultfstv";
		}else if($('form[name="jnittrsvproductVO"]').length>0){
			formName = "jnittrsvproductVO";
		}else if($('form[name="jnitedulecture"]').length>0){
			formName = "jnitedulecture";
		}else if($('form[name="jnitresearch"]').length>0){
				formName = "jnitresearch";
		}else if($('form[name="ctcult"]').length>0){
			formName = "ctcult";
		}else if($('form[name="sccult"]').length>0){
			formName = "sccult";
		}else if($('form[name="recruit"]').length>0){
			formName = "recruit";
		}
	}
	var config = {
		txHost: '<c:url value="" />', /* 런타임 시 리소스들을 로딩할 때 필요한 부분으로, 경로가 변경되면 이 부분 수정이 필요. ex) http://xxx.xxx.com */
		txPath: '/board/_editor/', /* 런타임 시 리소스들을 로딩할 때 필요한 부분으로, 경로가 변경되면 이 부분 수정이 필요. ex) /xxx/xxx/ */
		txService: 'sample', /* 수정필요없음. */
		txProject: 'sample', /* 수정필요없음. 프로젝트가 여러개일 경우만 수정한다. */
		initializedId: "", /* 대부분의 경우에 빈문자열 */
		wrapper: "tx_trex_container", /* 에디터를 둘러싸고 있는 레이어 이름(에디터 컨테이너) */
		form: formName+"", /* 등록하기 위한 Form 이름 */
		txIconPath: "<c:url value="/board/_editor/" />images/icon/editor/", /*에디터에 사용되는 이미지 디렉터리, 필요에 따라 수정한다. */
		txDecoPath: "<c:url value="/board/_editor/" />images/deco/contents/", /*본문에 사용되는 이미지 디렉터리, 서비스에서 사용할 때는 완성된 컨텐츠로 배포되기 위해 절대경로로 수정한다. */
		canvas: {
			styles: {
				color: "#123456", /* 기본 글자색 */
