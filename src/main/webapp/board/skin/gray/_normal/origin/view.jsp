							...
							<c:choose>						
								...
								<c:when test="${result.isSecret == 1}"><!-- 비밀글 -->
									...
									<c:if test="${result.mbrId == null || loginVO.mbrId == null}"><!-- 비회원글 -->
										<a href="?boardId=${boardinfoVO.boardId}&amp;mode=pw&amp;cntId=${result.id}&amp;category=${jtag:encodeUrl(result.category)}&amp;pageIdx=${param.pageIdx}" >
											<c:out value="${result.title}" />
										</a>
									</c:if>
									<c:if test="${result.mbrId != null &&  loginVO.mbrId != null}"><!-- 회원글 -->
										<c:if test="${result.mbrId == loginVO.mbrId || isAdmin == true}">
											<a href="?boardId=${boardinfoVO.boardId}&amp;mode=view&amp;cntId=${result.id}&amp;category=${jtag:encodeUrl(result.category)}&amp;pageIdx=${param.pageIdx}" >
												<c:out value="${result.title}" />
											</a>
										</c:if>
...
<div class="viewbtn">
	...
	<a id="linkEdit" class="btn" href="?boardId=${boardinfoVO.boardId}&amp;mode=edit&amp;cntId=${result.id}&amp;pageIdx=${param.pageIdx}"><%= JnitboardController.isLanguage("수정",lang) %></a>
	...
	<a class="btn" href="?boardId=${boardinfoVO.boardId}&amp;mode=list&amp;category=${category}&amp;pageIdx=${param.pageIdx}"><%= JnitboardController.isLanguage("목록",lang) %></a> 
</div>
