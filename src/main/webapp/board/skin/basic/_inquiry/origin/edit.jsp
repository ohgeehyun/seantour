...
<table class="bbsWrite table table-bordered" summary="<c:out value="${boardinfoVO.boardTitle}" /><%= JnitboardController.isLanguage("에 글을 수정하기 위한 입력폼으로",lang) %> <c:if test="${boardinfoVO.useCategory == '1' && empty fixedCategory}"><c:out value="${lbl.category}" />, </c:if><c:if test="${boardinfoVO.useSecret == '1'}"><%= JnitboardController.isLanguage("비밀글 여부",lang) %>, </c:if><c:forEach var="k" items="${sIdx}" varStatus="s"><c:choose><c:when test="${k == 'writer' || k == 'hit'}"><c:if test="${isAdmin == true}"><c:out value="${f[k].lbl}" /></c:if></c:when><c:otherwise><c:out value="${f[k].lbl}" /></c:otherwise></c:choose><c:out value="${s.last ? '' : ','}" /></c:forEach><c:if test="${boardinfoVO.useFile == 1}">, <%= JnitboardController.isLanguage("첨부",lang) %></c:if> <%= JnitboardController.isLanguage("으로 구성되어 있습니다.",lang) %>">
	...
	<tbody>
		...
		<c:if test="${isAdmin == true && boardinfoVO.useNotice == '1'}">
			<tr>
				<th style="width: 18%;"><label for="isNotice"><%= JnitboardController.isLanguage("공지글",lang) %></label></th>
				<td><input type="checkbox" name="isNotice" id="isNotice" value="1" <c:out value="${result.isNotice == '1' ? 'checked' : ''}"/>/></td>
			</tr>
		</c:if>
		<c:if test="${boardinfoVO.useSecret == '1' && loginVO.mbrId != null}">
			<tr>
				...
				<td><input type="checkbox" name="isSecret" id="isSecret" value="1" <c:out value="${result.isSecret == '1' ? 'checked' : ''}"/>/></td>
			</tr>
		</c:if>
		<c:forEach var="k" items="${sIdx}" varStatus="s">
			...
			<tr>
				...
				<td><input name="${k}" value="<c:out value="${result[k]}" />" id="${k}" class="boxstyle ${k} input-xxlarge" type="text" /></td>
			</tr>
			</c:if>
			</c:when>
			<%--created--%>
			...
			<tr>
				<c:if test="${param.pid == null || param.pid == ''}">
					...
					<td>
						<input name="${k}" value="<c:out value="${result[k]}" />" id="${k}" class="boxstyle ${k} input-xxlarge" type="text" required/>
					</td>
				</c:if>
				<c:if test="${param.pid != null && param.pid != ''}">
					<input name="title" value="<%= JnitboardController.isLanguage("답변입니다.",lang) %>" id="title" class="input-xxlarge boxstyle title" type="hidden" />
				</c:if>
			</tr>
			<%--비회원작성자 --%>
			...
			<tr>
				...
				<input name="${k}" id="${k}" value="<c:out value="${result[k]}" />" class="boxstyle ${k} input-xxlarge" type="text" <c:out value="${f[k].required?' required':''}"/>>
				</c:when>
				...
				<textarea id="${k}" rows="10" name="${k}" class="${k} input-xxlarge" <c:out value="${f[k].required?' required':''}"/>><c:out value="${result[k]}" /></textarea>
				</c:when>
				<%--address--%>
				<c:when test="${f[k].type == 'address'}">
				<input name="${k}" id="${k}" value="<c:out value="${result[k]}" />" class="address ${k} input-xxlarge" type="text" <c:out value="${f[k].required?' required':''}"/>>
...
<div class="writebtn">
	...
	<c:if test="${param.pid == null || param.pid == ''}">
		<a id="btnCancel" class="btn" href="?boardId=${boardinfoVO.boardId}&amp;mode=view&amp;cntId=${result.id}&amp;category=${category}"><%= JnitboardController.isLanguage("취소",lang) %></a>
	</c:if>
	<c:if test="${param.pid != null && param.pid != ''}">
		<a id="btnCancel" class="btn" href="?boardId=${boardinfoVO.boardId}&amp;mode=view&amp;cntId=<c:out value="${param.pid}" />&amp;category=${category}"><%= JnitboardController.isLanguage("취소",lang) %></a>
	</c:if>
	</div>
