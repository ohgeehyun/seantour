...
<div class="inquiry_answer_list">
	<c:forEach var="result" items="${inquiryList}" varStatus="status">
		<table class="bbsView table table-bordered inquiry_answer_list" summary="<c:out value="${boardinfoVO.boardTitle}" /><%= JnitboardController.isLanguage("의 조회 테이블로",lang) %><c:if test="${not empty lbl.writer}"><c:out value="${lbl.writer}" />,</c:if><c:if test="${not empty lbl.created}"><c:out value="${lbl.created}" />,</c:if><c:if test="${not empty lbl.hit}"><c:out value="${lbl.hit}" />,</c:if><c:if test="${not empty fileList && boardinfoVO.useFile != 0}" ><%= JnitboardController.isLanguage("첨부",lang) %><%--  --%>,</c:if><c:forEach var="k" items="${sIdx}" varStatus="status"><c:out value="${lbl[k]}" /><c:out value="${status.last ? '' : ','}" /></c:forEach><%= JnitboardController.isLanguage("으로 구성되어 있습니다.",lang) %>">
			<caption><c:out value="${boardinfoVO.boardTitle}" /> <%= JnitboardController.isLanguage("조회",lang) %></caption>
			<thead>
				<tr>
					<th colspan="6" class="title"><c:if test="${isAdmin == true && result.isdel == 1}"><span class="delArticle">(<%= JnitboardController.isLanguage("삭제글",lang) %>) </span></c:if><c:out value="${result.title}" /></th>
			...
			<tr>
				...
				<c:if test="${not empty lbl.created}">
				<th class="created"><c:out value="${lbl.created}" /></th>
				<td class="created">
					<fmt:parseDate var="dateFmt" value="${result.created}" pattern="yyyy-MM-dd" />
					<fmt:formatDate value="${dateFmt }" pattern="yyyy-MM-dd" />
				</td>
								...
								<c:forEach var="resultfile" items="${boardFileMap[result.id]}" varStatus="status2">																															
									<span class="list_file">
										<a href="javascript:Jnit_boardDownload('<c:url value="${resultfile.fileUrl}"/>','<c:url value="/board/ajax/file/downloadCount.do?fileId=${resultfile.fileId }"/>','${status.count }');" target="_self">
											...
											<c:out value="${resultfile.fileOnm}"/>
										</a>
									</span>
								</c:forEach>
			...
			<c:choose>
				...
				<td colspan="6" class="content">
					<div class="board_content">
					<c:if test="${result.isHtml == 1}">				
						<c:out value="${contentMap[result.id]}" escapeXml="false" />			
					</c:if>
					<c:if test="${result.isHtml == 0}">
		...
		<div class="viewbtn">
			<c:if test="${result.mbrId == loginVO.mbrId || isAdmin == true}">
				<a id="linkEdit" class="btn" href="?boardId=${boardinfoVO.boardId}&amp;mode=edit&amp;cntId=${result.id}&amp;pid=<c:out value="${param.cntId}" />&amp;pageIdx=<c:out value="${param.pageIdx}" />"><%= JnitboardController.isLanguage("수정",lang) %></a>
				<c:if test="${result.isdel == 0 || result.isdel == 2}">
			    <a id="linkDelete" class="btn" href="?boardId=${boardinfoVO.boardId}&amp;mode=proc&amp;proc=delete&amp;cntId=${result.id}&amp;pid=<c:out value="${param.cntId}" />&amp;retUrl=${retUrl}"><%= JnitboardController.isLanguage("삭제",lang) %></a>
			    </c:if>
			    <c:if test="${isAdmin }">
			    <a id="linkAllDelete" class="btn" href="?boardId=${boardinfoVO.boardId}&amp;mode=proc&amp;proc=allDelete&amp;cntId=${result.id}&amp;pid=<c:out value="${param.cntId}" />&amp;retUrl=${retUrl}"><%= JnitboardController.isLanguage("영구삭제",lang) %></a>
			    </c:if>	    
			</c:if>
		    <c:if test="${isAdmin == true}">
			    <c:if test="${result.isdel == 1}">
			    <a id="linkRecovery" class="btn" href="?boardId=${boardinfoVO.boardId}&amp;mode=proc&amp;proc=recovery&amp;cntId=${result.id}&amp;pid=<c:out value="${param.cntId}" />&amp;retUrl=${retUrl}"><%= JnitboardController.isLanguage("복구",lang) %></a>
			    </c:if>
			</c:if>   
		</div>
	</c:forEach>
