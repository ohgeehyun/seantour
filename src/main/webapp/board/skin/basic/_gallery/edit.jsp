	...
	<tbody>
		...
		<c:forEach var="k" items="${sIdx}" varStatus="s">
			...
			<tr>
				...
				<input name="${k}" id="${k}" value="<c:out value="${result[k]}" />" class="boxstyle ${k} input-xxlarge boxstyle" type="text" <c:out value="${f[k].required?' required':''}"/>>
				</c:when>
				...
				<textarea id="${k}" rows="10" name="${k}" class="${k} content input-xxlarge boxstyle" <c:out value="${f[k].required?' required':''}"/>><c:out value="${result[k]}" /></textarea>
				</c:when>
				<%--date yyyy-MM-dd--%>
		...
		<tr>
			<th><label>첨부</label> <span id="GalleryupfileAdd"><a href="#none">+</a></span> <span id="upfileDel"><a href="#none">-</a></span></th>
			<td>
