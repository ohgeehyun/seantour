...
<table class="bbsWrite table table-bordered" summary="<c:out value="${boardinfoVO.boardTitle}" /><c:out value="${boardWrite1 }"/><%-- 에 글을 작성하기 위한 입력폼으로 --%> <c:if test="${boardinfoVO.useCategory == '1' && empty fixedCategory}"><c:out value="${lbl.category}" />, </c:if><c:if test="${boardinfoVO.useSecret == '1'}"><c:out value="${boardWrite2 }"/><%-- 비밀글 여부 --%>, </c:if><c:forEach var="k" items="${sIdx}" varStatus="s"><c:choose><c:when test="${k == 'writer' || k == 'hit'}"><c:if test="${isAdmin == true}"><c:out value="${f[k].lbl}" /></c:if></c:when><c:otherwise><c:out value="${f[k].lbl}" /></c:otherwise></c:choose><c:out value="${s.last ? '' : ','}" /></c:forEach><c:if test="${boardinfoVO.useFile == 1}">, <c:out value="${boardWrite3 }"/><%-- 첨부 --%></c:if> <c:out value="${boardWrite16 }"/><%-- 으로 구성되어 있습니다. --%> ">
	<caption><c:out value="${boardinfoVO.boardTitle}" /> <c:out value="${boardWrite4 }"/><%-- 입력폼 --%></caption>
	<tbody>
