	...
	<tbody>
		...
		<c:forEach var="k" items="${sIdx}" varStatus="s">
			...
			<tr>
				...
				<td>
					<input name="${k}" value="" id="${k}" class="input-xxlarge boxstyle ${k}" type="text" required/>
				</td>
			</tr>
			</c:if>
			<c:if test="${pid != null && pid != ''}">
...
<div class="writebtn">
	...
	<c:if test="${pid != null && pid != ''}">
		<a id="btnCancel" class="btn" href="?boardId=${boardinfoVO.boardId}&amp;mode=view&amp;cntId=${pid}&amp;category=${category}"><%= JnitboardController.isLanguage("취소",lang) %></a>
	</c:if>
</div>
