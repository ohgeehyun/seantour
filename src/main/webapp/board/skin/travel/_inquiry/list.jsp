			...
			<tr>
				<c:forEach var="k" items="${sIdx}" varStatus="s">
					...
					<c:when test="${k == 'title'}">
						...
						<c:if test="${result.tmp3 == '1'}"><img src="<c:url value="/board/_common/img/new_icon.gif" />" alt="" /></c:if>
						<c:choose>						
						...
						<c:if test="${result.ext20 == '완료'}">
							<img src="<c:url value="/board/_common/img/answer_icon.gif" />" alt="답변" />
						</c:if>
