		...
		<c:choose>
			...
			<td colspan="6" class="content">
				<div class="board_content">				
					<c:forEach var="bbsfile" items="${fileList}" varStatus="status">
						<%
							Map rst = (Map)pageContext.getAttribute("bbsfile");			
							if(rst.get("fileUrl") != null && !"".equals(rst.get("fileUrl"))){
								String fileUrl = (String)rst.get("fileUrl");
								String[] fileUrlArr = fileUrl.split("/");
								fileUrlArr[fileUrlArr.length-1] = "thumb_" + fileUrlArr[fileUrlArr.length-1];
								fileUrl = StringUtils.join(fileUrlArr,"/");
								request.setAttribute("bbsfileUrl",fileUrl);
								
							}else{
								request.setAttribute("bbsfileUrl","/images/noimg.gif");
							}
						%>
						<%--
						<c:if test="${!empty mainFileVO && mainFileVO.ismain == '1'}"> 
						<div class="file_image">
							<img src="<c:url value="${bbsfileUrl}"/>" alt="<c:out value="${empty bbsfile.ext01 ? result.title : bbsfile.ext01}" /><c:out value="${empty bbsfile.ext01 ? ' boardView22' : ''}" />" />
						</div>
						</c:if> 
						--%>
					</c:forEach>	
