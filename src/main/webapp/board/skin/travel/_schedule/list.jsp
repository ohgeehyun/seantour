...
<div class="container-fluid">
    <div class="schedule row-fluid">
		...
		<!-- <td class="BorderRightNone"><a href="#">7</a></td>  -->
		<!--달력//-->
	    <div class="today_list span7 box">
	        ...
	        <ul class="todayList">
	            ...
	            <li>
					...
					<c:if test="${dbmsType != 'oracle' && dbmsType != 'tibero'}">
						<fmt:formatDate value="${result.date01 }" pattern="HH:mm" />
					</c:if>					
					<c:if test="${dbmsType == 'oracle' || dbmsType == 'tibero' }">
						<fmt:parseDate var="date01" value="${result.date01 }" pattern="yyyy-MM-dd HH:mm:ss" ></fmt:parseDate>
						<fmt:formatDate value="${date01}" pattern="HH:mm" />
					</c:if>
					</c:otherwise>
					</c:choose>
