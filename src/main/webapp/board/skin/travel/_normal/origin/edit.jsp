	...
	<tbody>
		...
		<c:forEach var="k" items="${sIdx}" varStatus="s">
			...
			<tr>
				...
				<input name="${k}" id="${k}" value="<c:out value="${result[k]}" />" class="input-xxlarge boxstyle boardEmail ${k} " type="text" <c:out value="${f[k].required?' required':''}"/>>
				</c:when>
				<%--textarea--%>
...
<div class="writebtn">
	<input type="submit" id="btnSubmit" class="btn" value="<%= JnitboardController.isLanguage("등록",lang) %>" />
