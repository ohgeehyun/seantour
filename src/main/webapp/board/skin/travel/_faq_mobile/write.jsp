...
<div class="m_boardWrap m_write normal">
	<table class="bbsWrite table table-bordered" summary="<c:out value="${boardinfoVO.boardTitle}" /><%= JnitboardController.isLanguage("에 글을 작성하기 위한 입력폼으로",lang) %><c:if test="${boardinfoVO.useCategory == '1' && empty fixedCategory}"><c:out value="${lbl.category}" />, </c:if><c:if test="${boardinfoVO.useSecret == '1'}"><%= JnitboardController.isLanguage("비밀글 여부",lang) %>, </c:if><c:forEach var="k" items="${sIdx}" varStatus="s"><c:choose><c:when test="${k == 'writer' || k == 'hit'}"><c:if test="${isAdmin == true}"><c:out value="${f[k].lbl}" /></c:if></c:when><c:otherwise><c:out value="${f[k].lbl}" /></c:otherwise></c:choose><c:out value="${s.last ? '' : ','}" /></c:forEach><c:if test="${boardinfoVO.useFile == 1}">, <%= JnitboardController.isLanguage("첨부",lang) %></c:if> <%= JnitboardController.isLanguage("으로 구성되어 있습니다.",lang) %>">
		...
		<tbody>
			...
			<%-- <c:if test="${isAdmin == true && boardinfoVO.useNotice == '1'}">
				<tr>
					<th scope="row"><label for="isNotice"><%= JnitboardController.isLanguage("공지글",lang) %></label></th>
			...
			<c:forEach var="k" items="${sIdx}" varStatus="s">
				...
				<tr>
					...
					<td><input name="${k}" id="${k}" class="m_boxstyle ${k} input-xxlarge" type="text" value="<c:out value='${loginVO.mbrNm}'/>"/></td>
				</tr>
				</c:if>
				</c:when>
				<c:when test="${ k == 'hit'}">
				...
				<tr>
					<th scope="row"><label for="${k}"><c:out value="${f[k].lbl}" /></label></th>
					<td><input name="${k}" value="" id="${k}" class="m_boxstyle ${k} input-xxlarge" type="text" /></td>
				</tr>
				</c:if>
				</c:when>
				<%--created--%>
				...
				<tr>
					<th scope="row"><label for="${k}"><c:out value="${f[k].lbl}" /></label></th>
					<td><input name="${k}" id="${k}" class="m_boxstyle  ${k} datetime" type="text" data-datetype="yyyy-MM-dd hh:mm:ss"/></td>
				</tr>
				</c:if>
				</c:when>
				<%--title--%>
				...
				<tr>
					...
					<td><input name="${k}" value="" id="${k}" class="m_m_boxstyle ${k}" type="text" required/></td>
				</tr>
				<%--비회원작성자 --%>
				...
				<tr>
					...
					<td><input name="writer" id="writer" class="m_boxstyle ${k}" required type="text" value="<c:out value='${loginVO.mbrNm}'/>"/></td>
				</tr>
				<tr>
					<th scope="row"><label for="<%= JnitboardController.isLanguage("비밀번호",lang) %>"><%= JnitboardController.isLanguage("비밀번호",lang) %></label></th>
					<td><input name="password" value="" id="password" class="m_boxstyle ${k}" required type="password" /></td>
				</tr>
				</c:if>
				</c:when>
				<%--content--%>
				...
				<tr>					
					<td colspan="2">
						...
						<c:if test="${boardinfoVO.useWyswyg != 1}">
							<textarea id="${k}" class="${k} content input-xxlarge m_boxstyle" rows="10" name="${k}" required></textarea>
						</c:if>
						<c:if test="${boardinfoVO.useWyswyg == 1}">						
				...
				<tr>
					...
					<input name="${k}" id="${k}" value="" class="input-xxlarge m_boxstyle ${k}" type="text" <c:out value="${f[k].required?' required':''}"/>>
					</c:when>
					...
					<c:otherwise>
					${f[k].type}<input name="${k}" id="${k}" value="" class="m_boxstyle ${k}" type="text" <c:out value="${f[k].required?' required':''}"/>>
					</c:otherwise>
					</c:choose>
					</td>
				</tr>
