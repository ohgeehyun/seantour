			...
			<c:when test="${k == 'content'}">
			 <div class="m_board_content">  
				...
				<div class="file_image">
					<img src="<c:url value="${mainFileVO.fileUrl}"/>" alt="<c:out value="${empty mainFileVO.ext01 ? result.title : mainFileVO.ext01}" /><c:out value="${empty mainFileVO.ext01 ? ' boardView22' : ''}" />" />
				</div>
				</c:if> --%>
				<c:forEach var="bbsfile" items="${fileList}" varStatus="status">
					<div class="file_image">
						<img src="<c:url value="${bbsfile.fileUrl}"/>" alt="<c:out value="${empty bbsfile.ext01 ? result.title : bbsfile.ext01}" /><c:out value="${empty bbsfile.ext01 ? ' boardView22' : ''}" />" />
					</div>
				</c:forEach>
